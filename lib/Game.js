 ...
 * @constructor
 */
function Game(id, type, name, decks, useVisio = false) {
    ...
    this.treasuresStack = Stack.createTreasureStack(decks);
    this.doorsStack = Stack.createDoorStack(decks);
 ...
 * @return {Game}
 */
Game.create = function(type, name, addons, useVisio) {

    // Load decks
    var decks = [require('./data/munchkin.json')];
    if(addons["munchkin-addon-2"]){
        decks.push(require('./data/munchkin-addon-2.json'));
    }
    if(addons["munchkin-addon-3"]){
        decks.push(require('./data/munchkin-addon-3.json'));
    }
    if(addons["munchkin-addon-4"]){
        decks.push(require('./data/munchkin-addon-4.json'));
    }
    if(addons["munchkin-addon-5"]){
        decks.push(require('./data/munchkin-addon-5.json'));
    }

    return new Game(shortid.generate(), type, name, decks, useVisio);
};

Game.createPrivate = function(data) {
    return Game.create(TYPE_PRIVATE, data.name, data.addon, data.useVisio);
};

Game.createPublic = function(data) {
    return Game.create(TYPE_PUBLIC, data.name, data.addon, data.useVisio);
};
...
Game.prototype.addPlayer = function(socket, data) {
    ...
    this.log("<strong>"+data.username + "</strong> a rejoint la partie");
...
Game.prototype.equipBoardCard = function(playerId, cardId){
    debug("Player "+playerId+" equip board card "+cardId);
    
    var player = this.players.get(playerId);
    var card = player.equipBoardCard(cardId);

    this.sockets.get(playerId).emit('board-card-list', player.getBoard());
    this.log("<strong>"+player.username+"</strong> a équipé <strong>" + card.name + "</strong>");
}

Game.prototype.unequipBoardCard = function(playerId, cardId){
    debug("Player "+playerId+" unequip board card "+cardId);
    
    var player = this.players.get(playerId);
    var card = player.unequipBoardCard(cardId);

    this.sockets.get(playerId).emit('board-card-list', player.getBoard());
    this.log("<strong>"+player.username+"</strong> a déséquipé <strong>" + card.name + "</strong>");
}
...
Game.prototype.increasePlayerLevel = function(playerId, givenPlayerId){
    ...
    this.log("<strong>"+player.username+"</strong> a passé " + "<strong>"+givenPlayer.username + "</strong> niveau <strong>"+level+"</strong>");
}
...
Game.prototype.increasePlayerGear = function(playerId, givenPlayerId){
    ...
    this.log("<strong>"+player.username+"</strong> a augmenté l'équipement de " + "<strong>"+givenPlayer.username + "</strong> à <strong>"+gear+"</strong>");
}
...
Game.prototype.decreasePlayerGear = function(playerId, givenPlayerId){
    ...
    this.log("<strong>"+player.username+"</strong> a diminué l'équipement de " + "<strong>"+givenPlayer.username + "</strong> à <strong>"+gear+"</strong>");
}
...
Game.prototype.disconnectPlayer = function(socketId){
    ...
    this.players.forEach(player => {
        if(player.socketId == socketId){
            ...
            this.log("<strong>"+player.username + "</strong> a quitté la partie");
        }
    });
}
