 ...
 * @constructor
 */
function Game(id, type, name, decks) {
 ...
 * @return {Game}
 */
Game.create = function(type, name, addons) {
    ...
    return new Game(shortid.generate(), type, name, decks);
};

Game.createPrivate = function(data) {
    return Game.create(TYPE_PRIVATE, data.name, data.addon);
};

Game.createPublic = function(data) {
    return Game.create(TYPE_PUBLIC, data.name, data.addon);
};
